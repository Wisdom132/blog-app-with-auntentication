...
exports.getBlogPosts = async (req, res) => {
	try {
		let response = await Blog.find()
			.populate('category')
			.select('tags dateCreated title content category.title category.description featured_image');
		res.status(200).json({
			data: response
		});
	} catch (err) {
...
exports.updateBlogData = async (req, res) => {
	try {
		const id = req.params.id;
		let response = await Blog.findByIdAndUpdate(id, req.body, { new: true });
		res.status(200).json({
			data: response
		});
	} catch (err) {
